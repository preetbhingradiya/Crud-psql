...
app.post("/todo/filter", async (req, res) => {
  try {
    let { name } = req.body;
    let sameNameTodo = await pool.query(queries.SAME_TODO_NAME, [`${name}%`]);
    console.log(sameNameTodo.rows);
    res.render("filter", { data: sameNameTodo.rows });
  } catch (error) {
    res.status(500).json({ error: error });
  }
});
